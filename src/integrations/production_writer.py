...
def _attach_target_dates(record: dict, calendar):
    raw = record.get("trading_date") or record.get("Trading Date")
    if not raw:
        return record
    trading_date = date.fromisoformat(str(raw)[:10])
    targets = horizon_targets(trading_date, calendar.is_trading_day)
    record["target_dates"] = {k: v.isoformat() for k, v in targets.items()}
    return record


def persist_prediction(payload: dict, extra: dict, notion_sink, calendar=None):
    """Validate, schedule, lock and write one canonical Prediction Registry record."""
    calendar = calendar or AShareTradingCalendar()
    record = build_record(payload, **extra)
    record["prediction"] = _canonical_forecast(record)
    _attach_target_dates(record, calendar)
    locked = lock_prediction(record)
    locked["Prediction Hash"] = locked["prediction_hash"]
    if "target_dates" in locked:
        locked["T+1 Target Date"] = locked["target_dates"]["T+1"]
        locked["T+3 Target Date"] = locked["target_dates"]["T+3"]
        locked["T+5 Target Date"] = locked["target_dates"]["T+5"]
...
def persist_production_bundle(
    ...
    notion = persist_prediction(
        payload, prediction_extra, notion_sink, calendar=calendar
    )
